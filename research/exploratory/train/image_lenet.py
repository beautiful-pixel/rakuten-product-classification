"""
LeNet-5 Canonical Training + Phase 3 Export

LeNet-5 adapted for Rakuten product image classification (224x224 RGB images).
This implementation follows the canonical training pipeline with Phase 3 exports
and Wandb integration.

Dependencies:
- torch, torchvision, numpy, pandas, sklearn, tqdm (standard ML stack)
- wandb (pip install wandb) - Required for experiment tracking

Local usage:
    python -m src.train.image_lenet --raw-dir data/raw --img-dir data/raw/images/image_train ...

Colab usage:
    python -m src.train.image_lenet --raw-dir /content/drive/.../data_raw --img-dir /content/images/... ...
"""
...
# Colab loader always available, local loader optional
...
class LeNetConfig:
    ...
    # Training
    img_size: int = 32                # LeNet typically uses 32x32
    batch_size: int = 128
    num_workers: int = 4
    num_epochs: int = 30
    lr: float = 1e-3
...
class LeNet5(nn.Module):
    """
    LeNet-5 adapted for Rakuten product classification.
    ...
    def __init__(self, num_classes: int = 27, dropout_rate: float = 0.5):
        ...
        # After conv1: (32-5+1)/1 = 28 -> maxpool -> 14
        # After conv2: (14-5+1)/1 = 10 -> maxpool -> 5
        # Feature size: 16 * 5 * 5 = 400
...
def _build_transforms(img_size: int) -> Tuple[transforms.Compose, transforms.Compose]:
    """Build train and validation transforms for LeNet."""
...
def _build_lenet(
    ...
    """
    Build LeNet-5 model for canonical classes.

    Args:
        num_classes: Number of output classes (27 for canonical)
        cfg: LeNetConfig with model parameters
...
def _make_loaders(
    ...
    Uses full_dataset + IndexedDataset to preserve global idx semantics.
    """
...
def _train_one_epoch(
    ...
    for batch in pbar:
        # Handle IndexedDataset return (img, label, real_idx)
        if len(batch) == 3:
...
def run_lenet_canonical(cfg: LeNetConfig) -> Dict[str, Any]:
    """
    Canonical LeNet training + Phase 3 export + B4-verifiable contract.
    Includes AdamW optimizer and ReduceLROnPlateau scheduler.
    """
    ...
    # 4) DataLoaders (returns full_dataset for export reuse)
    ...
    # Reuse full_dataset_val for export
    ...
    # y_true must be canonical indices (0..26), not original labels
    ...
    export_result = export_predictions(
        ...
        y_true=y_true,
        extra_meta={
            "source": "src/train/image_lenet.py",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LeNet-5 Canonical Training + Export")
    ...
    parser.add_argument("--ckpt-dir", type=str, default="checkpoints/image_lenet", help="Checkpoint directory")
    ...
    parser.add_argument("--export-name", type=str, default="lenet_v1", help="Model name for export")
    ...
    print("="*80)
    print("LeNet-5 Canonical Training Configuration")
    print("="*80)
