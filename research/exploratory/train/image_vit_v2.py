"""
ViT Canonical Training + Phase 3 Export + WandB Integration

Dependencies:
- timm (pip install timm) - Required for Vision Transformer models
- torch, torchvision, numpy, pandas, sklearn, tqdm (standard ML stack)
- wandb (pip install wandb) - Experiment tracking

Local usage:
    python -m src.train.image_vit --raw-dir data/raw --img-dir data/raw/images/image_train --wandb-project rakuten-vit ...

Colab usage:
    python -m src.train.image_vit --raw-dir /content/drive/... --wandb-project rakuten-vit ...
"""
...
class ViTConfig:
    ...
    # Training
    img_size: int = 224
    batch_size: int = 32              # REDUCED default from 64 to 32 for ViT-Base memory safety
    num_workers: int = 4
    num_epochs: int = 1
    lr: float = 1e-4
    use_amp: bool = True
    ...
    # Model
    # UPDATED: Changed default from 'tiny' to 'base' for better performance
    ...
    model_name: str = "vit_base_canonical_v1"  # Export name
...
def _build_vit(num_classes: int, dropout_rate: float, model_name: str, pretrained: bool = True) -> nn.Module:
    """
    Build ViT using timm with custom head for canonical classes.
    """
    try:
        import timm
    except ImportError:
        raise ImportError(
            "timm is required for ViT models.\n"
            "Install with: pip install timm"
        )

    print(f"[INFO] Loading model: {model_name} (pretrained={pretrained})")
...
def _make_loaders(
    ...
    # Use IndexedDataset to wrap full_dataset with split indices
...
def run_vit_canonical(cfg: ViTConfig) -> Dict[str, Any]:
    """
    Canonical ViT training + Phase 3 export + B4-verifiable contract.
    """
    ...
    if cfg.force_colab_loader:
        ...
        pack = load_data_colab(raw_dir=cfg.raw_dir, img_root=Path(cfg.img_dir), splitted=False, verbose=True)
        X, y = pack["X"], pack["y"]
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="max", factor=cfg.plateau_factor, patience=cfg.plateau_patience
    )
    ...
    for epoch in range(int(cfg.num_epochs)):
        train_loss, train_acc, train_f1 = _train_one_epoch(
            ...
            optimizer=optimizer, device=device, use_amp=use_amp, scaler=scaler
        )
        ...
        print(
            ...
            f"T_Loss={train_loss:.4f} T_F1={train_f1:.4f} | "
            f"V_Loss={val_loss:.4f} V_F1={val_f1:.4f} | "
            f"lr={current_lr:.2e}"
        )
    ...
    export_result = export_predictions(
        ...
        y_true=y_true,
        extra_meta={
            "source": "src/train/image_vit.py",
            "model_architecture": f"timm.{cfg.vit_model_name}",
            "wandb_id": wandb.run.id if cfg.use_wandb and wandb.run else None,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ViT Canonical Training + Export + WandB")
    ...
    parser.add_argument("--ckpt-dir", type=str, default="checkpoints/image_vit", help="Checkpoint directory")

    parser.add_argument("--epochs", type=int, default=1, help="Number of epochs")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size (lowered for ViT Base)")
    parser.add_argument("--num-workers", type=int, default=2, help="DataLoader workers")
    parser.add_argument("--lr", type=float, default=1e-4, help="Learning rate")

    # Model Selection: Default changed to Base
    ...
    # WandB Configuration
    parser.add_argument("--wandb-project", type=str, default="rakuten-image-classification", help="WandB Project Name")
    ...
    parser.add_argument("--force-colab-loader", dest="force_colab_loader", action="store_true",
                        help="Force load_data_colab(raw_dir=...)")
    ...
    parser.add_argument("--export-name", type=str, default="vit_base_canonical", help="Model name for export")
    ...
    print("="*80)
    print(f"ViT Training (Model: {cfg.vit_model_name})")
    print("="*80)
    print(f"WandB: {cfg.use_wandb} (Project: {cfg.wandb_project})")
