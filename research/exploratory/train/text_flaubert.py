"""
FlauBERT Canonical Training + Phase 3 Export
...
Local usage:
    python -m src.train.text_flaubert --raw-dir data/raw --out-dir artifacts/exports ...

Colab usage:
    python -m src.train.text_flaubert --raw-dir /content/drive/.../data_raw ...
"""
...
class FlauBERTConfig:
    ...
    # Model
    model_name: str = "flaubert/flaubert_base_cased"  # or "flaubert/flaubert_large_cased"
    ...
    export_name: str = "flaubert_canonical"  # Export name
...
def run_flaubert_canonical(cfg: FlauBERTConfig) -> Dict[str, Any]:
    """
    Canonical FlauBERT training + Phase 3 export + B4-verifiable contract.
    ...
    # ======================================================================
    # 2) CRITICAL: Load canonical splits (single source of truth)
    # ======================================================================
    ...
    # ======================================================================
    # 5) CRITICAL: Split data using canonical indices
    # ======================================================================
    ...
    # CRITICAL: FlauBERT doesn't have fast tokenizer!
    use_fast = "flaubert" not in cfg.model_name.lower()
    print(f"[INFO] Using fast tokenizer: {use_fast}")
    tokenizer = AutoTokenizer.from_pretrained(cfg.model_name, use_fast=use_fast)

    print(f"[INFO] Loading model: {cfg.model_name}")
    # CRITICAL: FlauBERT uses different parameter names than BERT (based on XLM)
    model = AutoModelForSequenceClassification.from_pretrained(
        cfg.model_name,
        num_labels=len(CANONICAL_CLASSES),
        dropout=cfg.dropout,                   # FlauBERT uses 'dropout' not 'hidden_dropout_prob'
        attention_dropout=cfg.dropout,         # FlauBERT uses 'attention_dropout' not 'attention_probs_dropout_prob'
        summary_first_dropout=cfg.dropout,     # Dropout for the classification head
    )
    ...
    export_result = export_predictions(
        ...
        y_true=y_true,
        extra_meta={
            "source": "src/train/text_flaubert.py",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="FlauBERT Canonical Training + Export")
    ...
    parser.add_argument("--ckpt-dir", type=str, default="checkpoints/text_flaubert", help="Checkpoint directory")
    ...
    # Model selection
    parser.add_argument("--model", type=str, default="flaubert/flaubert_base_cased",
                        help="HuggingFace model name (default: flaubert/flaubert_base_cased)")
    ...
    parser.add_argument("--export-name", type=str, default="flaubert_canonical", help="Model name for export")
    ...
    print("="*80)
    print("FlauBERT Canonical Training Configuration")
    print("="*80)
