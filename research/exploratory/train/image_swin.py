...
class RakutenSwin(nn.Module):
    """Swin Transformer for Rakuten classification with custom head and stochastic depth."""
...
def _build_transforms(img_size: int) -> Tuple[transforms.Compose, transforms.Compose]:
    """Build train and validation transforms for Swin."""
...
def _train_one_epoch(
    ...
    """Train one epoch with Mixup/CutMix support."""
    model.train()
    total_loss = 0.0
    all_preds = []
    all_labels = []
...
def run_swin_canonical(cfg: SwinConfig) -> Dict[str, Any]:
    ...
    # 8) Export predictions (alignment-safe) on cfg.export_split
    ...
    export_result = export_predictions(
        ...
        extra_meta={
            ...
            "model_architecture": f"timm.{cfg.swin_model_name}",
